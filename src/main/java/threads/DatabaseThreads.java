...
public abstract class DatabaseThreads {
    ...
    public synchronized void saveCamera(Camera camera, Change change){
        ...
        DatabaseUtil.saveCamera(camera);
        DatabaseUtil.saveChange(change);
        isDatabaseOperationInProgress=false;
        notifyAll();
    }

    public synchronized void saveCO2Sensor(CO2Sensor co2Sensor, Change change){
        while(isDatabaseOperationInProgress){
            try{
                wait();
            } catch(InterruptedException ex){
        ...
        isDatabaseOperationInProgress=true;
        DatabaseUtil.saveCO2Sensor(co2Sensor);
        DatabaseUtil.saveChange(change);
        isDatabaseOperationInProgress=false;
        notifyAll();
    }
    public synchronized void saveGlassBreakSensor(GlassBreakSensor glassBreakSensor, Change change){
        while(isDatabaseOperationInProgress){
            try{
                wait();
            } catch(InterruptedException ex){
        ...
        isDatabaseOperationInProgress=true;
        DatabaseUtil.saveGlassBreakSensor(glassBreakSensor);
        DatabaseUtil.saveChange(change);
        isDatabaseOperationInProgress=false;
        notifyAll();
    }
    public synchronized void saveHeatSensor(HeatSensor heatSensor, Change change){
        while(isDatabaseOperationInProgress){
            try{
                wait();
            } catch(InterruptedException ex){
        ...
        isDatabaseOperationInProgress=true;
        DatabaseUtil.saveHeatSensor(heatSensor);
        DatabaseUtil.saveChange(change);
        isDatabaseOperationInProgress=false;
        notifyAll();
    }
    public synchronized void saveMotionSensor(MotionSensor motionSensor, Change change){
        while(isDatabaseOperationInProgress){
            try{
                wait();
            } catch(InterruptedException ex){
        ...
        isDatabaseOperationInProgress=true;
        DatabaseUtil.saveMotionSensor(motionSensor);
        DatabaseUtil.saveChange(change);
        isDatabaseOperationInProgress=false;
        notifyAll();
    }
    public synchronized void saveSmokeSensor(SmokeSensor smokeSensor, Change change){
        while(isDatabaseOperationInProgress){
            try{
                wait();
            } catch(InterruptedException ex){
        ...
        isDatabaseOperationInProgress=true;
        DatabaseUtil.saveSmokeSensor(smokeSensor);
        DatabaseUtil.saveChange(change);
        isDatabaseOperationInProgress=false;
        notifyAll();
    }

    public synchronized  void saveChange(Change change){
        while(isDatabaseOperationInProgress){
            try{
                wait();
            } catch(InterruptedException ex){
